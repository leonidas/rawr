  ...
  Chart = (function() {
    ...
    Chart.prototype.updateDataLayer = function(data, styles, layerName) {
      ...
      rect.style("position", "absolute").text(function(d) {
        ...
        return Math.round(_this.width - xScale(d.__startX__ + d.width));
      ...
      rect.enter().append("div").attr("class", "rect").attr("style", function(d) {
        ...
        return Math.round(_this.width - xScale(d.__startX__ + d.width));
      }).style("top", this.height - this.margin).style("bottom", this.margin).text(function(d) {
        if (d.__indexWithinGroup__ === 0) {
          return d.title;
        } else {
          return "";
        }
      }).transition().delay(500).duration(500).style("top", function(d) {
        return Math.round(yScale(d.height));
      });
