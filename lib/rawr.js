(function() {
  ...
  Chart = (function() {
    ...
    Chart.prototype.getChartCanvas = function(className) {
      var _base;
      if ((_base = this.chartCanvases)[className] == null) {
        _base[className] = this.parent.append('svg:svg').classed("chartCanvas", true).style("position", "absolute").style("left", 0).style("right", 0).style("width", this.width).style("height", this.height);
      }
      console.log(this.chartCanvases[className]);
      return this.chartCanvases[className];
    };
