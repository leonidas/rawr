(function() {
  ...
  Chart = (function() {
    ...
    Chart.prototype.drawXLabels = function() {
      this.xLabels = this.chart.selectAll(".x-label").data(this.xScale.ticks(10), String);
      ...
      this.xLabels.transition().attr("x", this.xScale);
      return this.xLabels.exit().transition().attr("x", this.xScale).style("opacity", "0").remove();
    };

    Chart.prototype.drawYLabels = function() {
      this.yLabels = this.chart.selectAll(".y-label").data(this.yScale.ticks(10), String);
      ...
      this.yLabels.transition().attr("y", this.yScale);
      return this.yLabels.exit().transition().attr("y", this.yScale).style("opacity", "0").remove();
    };

    Chart.prototype.drawDataRectangles = function(data, styles, className) {
      var addIndexWithinGroup, addStartingX, labelY, newRectG, rectG, xScale, yScale;
      ...
      console.log(data);
      rectG = this.chart.selectAll('.' + className).data(data, function(d) {
      ...
      newRectG.append("rect").attr("style", function(d) {
        ...
        return yScale(0);
      }).transition().delay(250).attr("x", function(d) {
        return xScale(d.__startX__);
      }).attr("width", function(d) {
        return xScale(d.__startX__ + d.width) - xScale(d.__startX__);
      }).attr("y", function(d) {
