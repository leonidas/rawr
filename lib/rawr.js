(function() {
  ...
  Chart = (function() {
    ...
    function Chart(where, width, height, margin) {
      ...
      this.chartCanvas = this.parent.append('svg:svg').classed("chartCanvas", true).style("position", "absolute").style("left", 0).style("right", 0).style("width", this.width).style("height", this.height);
    }
    ...
    Chart.prototype.drawDataRectangles = function(data, styles, className) {
      ...
      console.log(data);
      rectG = this.chartCanvas.selectAll('.' + className).data(data, function(d) {
