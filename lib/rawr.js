(function() {
  ...
  Chart = (function() {
    ...
    Chart.prototype.updateDataLayer = function(data, styles, layerName) {
      var addIndexWithinGroup, addStartingX, chartCanvas, labelY, rect, xScale, yScale;
      ...
      rect.style("position", "absolute").transition().duration(500).style("left", function(d) {
        return Math.round(xScale(d.__startX__));
      }).style("width", function(d) {
        return Math.round(xScale(d.__startX__ + d.width) - xScale(d.__startX__));
      ...
      rect.enter().append("div").attr("class", "rect").attr("style", function(d) {
        ...
        return Math.round(xScale(d.__startX__ + d.width) - xScale(d.__startX__));
      }).style("top", this.height - this.margin).style("bottom", this.margin).transition().delay(500).duration(500).style("left", function(d) {
        return Math.round(xScale(d.__startX__));
      }).style("width", function(d) {
        return Math.round(xScale(d.__startX__ + d.width) - xScale(d.__startX__));
      }).style("top", function(d) {
