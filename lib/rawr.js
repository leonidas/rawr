(function() {
  ...
  Chart = (function() {
    ...
    Chart.prototype.drawXLabels = function() {
      return this.chart.selectAll(".x-label").data(this.xScale.ticks(10)).enter().append("svg:text").attr("class", "x-label").attr("x", this.xScale).attr("y", this.height - this.margin + 15).attr("text-anchor", "middle").text(String);
    };

    Chart.prototype.drawYLabels = function() {
      return this.chart.selectAll(".y-label").data(this.yScale.ticks(10)).enter().append("svg:text").attr("class", "y-label").attr("x", this.margin - 5).attr("y", this.yScale).attr("text-anchor", "end").attr("alignment-baseline", "middle").text(String);
    };
    ...
    Chart.prototype.drawDataRectangles = function(data, styles, className) {
      ...
      yScale = this.yScale;
      console.log(data);
      this.rectG = this.chart.selectAll('.' + className).data(data).enter().append("svg:g");
      return this.rectG.append("svg:rect").attr("class", className).attr("x", function(d) {
    ...
    Chart.prototype.drawRectangleLabels = function(data) {
      ...
      return this.rectG.append("svg:g").attr("class", "box-label").attr("transform", function(d) {
