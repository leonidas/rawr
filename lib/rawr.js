(function() {
  ...
  Chart = (function() {
    ...
    Chart.prototype.getChartCanvas = function(layerName) {
      var _base, _ref;
      return (_ref = (_base = this.chartCanvases)[layerName]) != null ? _ref : _base[layerName] = this.parent.append('svg:svg').classed("chartCanvas", true).style("position", "absolute").style("left", 0).style("right", 0).style("width", this.width).style("height", this.height);
    };
    ...
    Chart.prototype.drawDataRectangles = function(data, styles, layerName) {
      ...
      if (layerName == null) layerName = 'layer1';
      ...
      addStartingX(data);
      console.log(data);
      chartCanvas = this.getChartCanvas(layerName);
      xScale = this.xScale;
      yScale = this.yScale;
      labelY = this.height - this.margin - 3;
      rectG = chartCanvas.selectAll('.' + layerName).data(data, function(d) {
      ...
      newRectG = rectG.enter().append("g").attr("class", layerName);
